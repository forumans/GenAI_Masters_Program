...
# Page configuration
st.set_page_config(
    page_title="AutoGen Intelligent Feedback Analysis System",
    page_icon="�",
    layout="wide",
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        # Initialize AutoGen system in session state
        if 'autogen_system' not in st.session_state:
            st.session_state.autogen_system = AutoGenFeedbackAnalysisSystem()
        
        # Process new data button
        if st.button("🚀 Process New Data", type="primary"):
            with st.spinner("Processing feedback data with AutoGen..."):
                try:
                    results = st.session_state.autogen_system.process_feedback()
                    st.session_state.processing_results = results
                    st.success("AutoGen processing completed successfully!")
                    st.rerun()
...
def dashboard_page():
    """Main dashboard page"""
    st.markdown('<h1 class="main-header">🤖 AutoGen System Dashboard</h1>', unsafe_allow_html=True)
...
def system_status_page():
    ...
    # Check if AutoGen system exists
    if 'autogen_system' in st.session_state:
        status = st.session_state.autogen_system.get_system_status()
